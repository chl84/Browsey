    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'Historical Linux baseline: Fedora 43; recent desktop checks: Arch/Omarchy. See release notes for validation scope.',
      ...
      },
      {
        ...
        body: 'This docs site is a standalone Svelte project in docs-site/, deployed with GitHub Pages. Project operations documents live in docs/.',
        bullets: [
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Linux phones: install the GVFS MTP backend, unlock the phone, and enable USB file transfer',
          'Linux USB formatting: UDisks2, a PolicyKit authentication agent, and exfatprogs/dosfstools/e2fsprogs/btrfs-progs for the corresponding filesystem',
        ],
      },
      {
      ...
      },
      {
        ...
        bullets: [
          ...
          '6. Start Browsey, enable cloud in `Settings > Cloud`, and use the status block plus `Test connection` there if you want to verify the current setup state and runtime health.',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Interactive cloud folder loads use shorter read timeouts than background refresh so stalled rc reads degrade to CLI within a user-facing wait budget.',
          'Interactive cloud folder loads can be cancelled from the Browsey activity pill while the folder is opening.',
      ...
      },
      {
        ...
        bullets: [
          ...
          'If rc startup or rc reads fail, Browsey now warns in backend logs and falls back to CLI automatically for interactive folder opens.',
          'Use `Settings > Cloud > Test connection` to compare the selected remote through rc and CLI explicitly, instead of guessing which path is broken.',
          'Use Browsey debug health command (`cloud_rc_health`) to inspect rc enabled state and daemon/socket status.',
          'If a cloud folder appears stuck, use activity cancel first, then inspect `browsey/logs/browsey.log` with log level set to `Debug`.',
          'If you suspect an rc daemon/socket issue, retry with `BROWSEY_RCLONE_RC=0` to force CLI-only reads and compare behavior.',
        ],
      },
      {
        id: 'run-dev',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Desktop release binary with embedded frontend: frontend/node_modules/.bin/tauri build --no-bundle (do not distribute a plain cargo build --release binary)',
          'Windows NSIS: cargo tauri build --bundles nsis',
          'Linux RPM + DEB: cargo tauri build --bundles rpm,deb',
        ],
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'rclone cloud paths support cloud↔cloud and mixed local↔cloud copy/move in v1 (with current cloud limitations noted below)',
          'Long transfers run in background with progress and cancel support',
          'Mixed cloud/local file transfers show real byte progress when rclone rc progress data is available, including aggregated progress for multi-file file batches',
          'Entries can be dropped onto bookmark targets in the sidebar, using the same copy/move routing as breadcrumb drops',
        ],
      },
      {
        id: 'recent-flow',
        title: 'Recent View',
        bullets: [
          'Recent entries are read from the app database and validated when the view opens',
          'Dead recent entries are pruned automatically during listing instead of lingering in the database indefinitely',
          'Slow network/GVFS recent paths use bounded probing so stale remote paths do not block Recent view startup for long periods',
        ],
      },
      {
        id: 'duplicate-flow',
      ...
      },
      {
        ...
        bullets: [
          'Supports Zip, Tar variants, GZ/BZ2/XZ/Zstd, 7z, and compressed RAR4/RAR5 through a streaming UnRAR adapter',
          ...
          'Complete multi-volume RAR sets are supported; missing volumes and password-protected archives report explicit errors',
        ],
      },
      {
        ...
        bullets: [
          ...
          'Delete and trash progress is measured in items, not bytes (including phone files)',
        ],
      },
      {
      ...
      },
      {
        ...
        bullets: [
          ...
          'Sidebar bookmarks can also be used as drag/drop destinations for copy/move operations',
        ],
      },
      {
      ...
      },
      {
        ...
        bullets: [
          ...
          'No filesystem watch/live refresh for `rclone://` paths (Browsey uses explicit/manual refresh and background refresh in selected flows)',
        ],
      },
      {
        id: 'open-with-flow',
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'Visible cards take priority over prefetch; obsolete work is cancelled on scrolling/navigation and completed results are reused',
          'Disk-cache hits validate the source path and metadata without opening original file contents; eviction follows the configured byte budget and recent use',
          'Ctrl + wheel changes from list to five grid sizes: 64, 96, 128, 160, and 192 CSS px; zoom is window-local and resets on restart',
          'Zoom requests sharper thumbnails while retaining the view anchor. Grid gaps are 8 px (Cozy) or 6 px (Compact) on both axes',
          'Cloud Grid thumbnails (`rclone://`) are opt-in and limited to image/pdf/svg with prechecks (allowed extension, known size, <= 50 MB)',
          'Cloud thumbnail materialization reuses managed cloud-open cache entries with in-flight dedupe and waiter timeout protection',
      ...
      },
      {
        ...
        bullets: [
          ...
          'MTP phones are discovered through GIO without launching another file manager and mount on demand; unlock the phone and select USB file transfer',
          'Phone Properties are informational; phone formatting and POSIX permission editing are not offered',
          'Unmounted removable USB partitions offer Mount and open; mounted drives expose Properties',
          'USB Format offers exFAT, FAT32, ext4, and btrfs when system tools are available; the operation is destructive and requires confirmation',
          'New ext4/btrfs USB filesystems grant ownership to the formatting user; existing volumes are not silently re-owned',
          'Formatting uses real UDisks progress when available, otherwise indeterminate progress; uncertain completion requires inspection, never an automatic retry',
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Theme mode: system/Omarchy colors, light, or dark; system mode falls back to the OS color scheme when no palette is available',
          'System-theme lasso uses a separate 16% opacity fill; selected-file highlighting is unchanged',
          'Archive defaults (name, compression level, open-destination-after-extract)',
          'Thumbnail settings (video thumbnails enabled flag, cloud thumbs enabled flag, ffmpeg path override, cache size)',
          'Mount list polling interval',
          'Double-click speed for mouse-based opening in list/grid',
          'Hardware acceleration toggle',
          'High contrast, scrollbar width, rclone path, and log level',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Log level: warn',
          'Rclone path: empty (auto-detect)',
        ],
      },
      {
        id: 'restore-defaults',
        title: 'Restore Defaults Behavior',
        bullets: [
          'Settings includes a `Restore defaults` action in the modal header.',
          'The action requires explicit confirmation before anything is changed.',
          'A confirmed restore resets all persisted settings to the default values listed above.',
          'A confirmed restore also resets all remappable keyboard shortcuts back to their default bindings.',
        ],
      },
      {
        id: 'settings-ranges',
      ...
      },
      {
        ...
        bullets: [
          'Settings > Data provides clear actions for thumbnail cache, cloud file cache, stars, bookmarks, and recents',
          ...
          'Clearing cloud file cache removes cached local copies of files opened from cloud remotes',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Ctrl+B: open the bookmark action for a single selected folder',
          ...
          'Delete / Shift+Delete: move to wastebasket / permanent delete',
          'Ctrl+R: rename',
        ],
      },
      {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'frontend/src/shared/ui/ contains reusable input primitives such as `TextField` and `ComboBox`, including modal-safe dropdown behavior',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Cloud-open cache stores local copies of opened cloud files and can be cleared from Settings > Data',
...
capabilities/default.json`,
  ...
  },
  {
    ...
    sections: [
      ...
      {
        id: 'v102',
        title: 'v1.0.2 (2026-09-25)',
        bullets: [
          'Linux USB formatting to exFAT/FAT32/ext4/btrfs, user ownership on new Linux filesystems, drive Properties, device-scoped progress, and safer uncertain-completion handling',
          'GIO MTP phone discovery and on-demand mounting without opening another file manager; phone-specific properties and unlock guidance',
          'Visible-first thumbnails, stale-work cancellation, cache reuse, bounded workers, JPEG improvements, and byte-budget/recent-use cache eviction',
          'Ctrl + wheel zoom from list through five grid sizes (64–192 px), sharper thumbnails, retained view anchor, and 8/6 px Cozy/Compact spacing',
          'System/Omarchy themes in Settings, with a lighter 16% lasso fill independent of selected-file highlights',
          'Delete/trash progress counts items rather than incorrectly displaying bytes',
          'Streaming UnRAR adapter for compressed RAR4/RAR5 and complete multi-volume sets, retaining safety limits and cancellation',
          'Keyboard-accessible menus, improved dialog focus and error retention, and expanded regression coverage',
          'Linux x86_64 RPM and DEB release scope; Windows remains in maintenance mode with no new installer in this release',
        ],
        note: 'See CHANGELOG.md and docs/releases/1.0.2.md in the repository for full changes, validation evidence, and limitations. Previous GitHub assets were v0.4.6; this release also includes the documented 1.0.0 and 1.0.1 changes.',
      },
      {
        id: 'v101',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Added opt-in Cloud thumbs for Grid thumbnails on rclone paths (image/pdf/svg only in v1, cloud video thumbnails intentionally blocked)',
          'Cloud thumbnail flow now checks thumbnail-cache hits before cloud materialization, reducing repeated download attempts across folder navigation',
          'Cloud-open cache prune behavior was hardened to avoid dropping managed cloud files while associated metadata is still fresh',
          'Cloud-thumbnail runtime failures now use explicit typed-error mapping for cloud command failures (instead of message-pattern fallback only)',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Mixed local-to-cloud writes now invalidate cloud listing cache correctly, and Recent view now prunes dead entries while bounding slow stale-remote probes',
          'Entries can now be dropped onto bookmark targets in the sidebar with the same copy/move behavior as breadcrumb drops',
          'Improved cloud UX/performance with background refresh for cloud write operations, refresh coalescing, reduced conflict-preview metadata calls, cloud remote/listing caches with invalidation, and bounded per-remote concurrency with metadata retry/backoff',
          'Hardened cloud routing and navigation: `rclone://` paths no longer fall into local FS/undo/GVFS code paths, cloud sort clicks avoid remote reloads, and breadcrumbs/direct navigation now handle `rclone://` paths correctly',
          'Removed OneDrive-specific GVFS backend support in favor of rclone-backed cloud integration and renamed generic Linux mount helpers from `gvfs.rs` to `gio_mounts.rs`',
      ...
      },
      {
        ...
        bullets: [
          ...
          'GVFS/MTP reliability improved for mounts and transfer flows',
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'When a cloud folder open is slow or degraded, use the activity cancel action first instead of force-closing the app',
          'If cloud reads keep falling back or timing out, compare normal startup with `BROWSEY_RCLONE_RC=0` to separate CLI issues from rc daemon issues',
        ],
      },
      {
        ...
        bullets: [
          ...
          'For support/debug capture, set Settings > Advanced > Log level to `Debug`, reproduce the issue, then attach `browsey.log` (and `browsey.log.1` if present)',
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'For support/debug capture, temporarily set Settings > Advanced > Log level to Debug before reproducing the issue',
          'With Info/Debug logging enabled, Browsey records startup context such as app version, profile, target OS/arch, and log file path',
