...
const hasScrollableOverflowY = (el: HTMLElement) => {
  const overflowY = getComputedStyle(el).overflowY
  return overflowY === 'auto' || overflowY === 'scroll' || overflowY === 'overlay'
}

const isScrollableVertically = (el: HTMLElement) =>
  el.scrollHeight > el.clientHeight && hasScrollableOverflowY(el)

const findNearestScrollContainer = (
  boundary: HTMLElement,
  target: EventTarget | null,
): HTMLElement | null => {
  let node: Node | null = target instanceof Node ? target : boundary
  while (node) {
    if (node instanceof HTMLElement && boundary.contains(node) && isScrollableVertically(node)) {
      return node
    }
    if (node === boundary) {
      break
  ...
  return isScrollableVertically(boundary) ? boundary : null
}
...
export const applyWheelScrollAssist = (
  ...
  if (isLikelyWheel && shouldEnforceMinStep && Math.abs(step) < cfg.minWheelStepPx) {
    step = Math.sign(deltaY) * cfg.minWheelStepPx
  }
  step = clampAbs(step, cfg.maxStepPx)
...
export const applyContainedWheelScrollAssist = (
  boundary: HTMLElement,
  event: WheelEvent,
) => {
