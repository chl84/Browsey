...
test('Open with saves a file-type default explicitly and keeps save errors retryable', async ({ page }) => {
  await page.goto('/')
  await openFileMenu(page)
  await page.getByRole('menuitem', { name: 'Open with…', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Open with…' })
  const save = dialog.getByRole('button', { name: 'Set as default', exact: true })
  await expect(save).toBeDisabled()
  await dialog.getByPlaceholder('Filter apps').fill('Beta')
  await expect(dialog.getByText('Set as default applies to all files of type')).toContainText('text/plain')
  ...
  await expect(dialog).not.toBeVisible()
  const { calls } = await page.evaluate(control)
  expect(calls.some((call) => call.cmd === 'open_with')).toBe(false)
