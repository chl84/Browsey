...
1. Expand multi-item local copy/move hostile-condition coverage beyond the added file+directory mid-batch-cancel, file+directory later-source-failure, recursive merge-cancel, and recursive merge-failure rollback paths, especially around any remaining summary/reporting-sensitive edges.
