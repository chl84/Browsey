...
| `CO-MTC-005` | Partial | `mixed_execute_local_to_cloud_directory_copy_and_move_via_fake_rclone`, `mixed_execute_cloud_to_local_directory_copy_and_move_via_fake_rclone`, `mixed_execute_local_to_cloud_partial_directory_move_invalidates_cache_and_keeps_partial_state`, `mixed_execute_local_to_cloud_directory_copy_cancels_during_second_active_transfer`, `mixed_execute_cloud_to_local_directory_copy_cancels_during_second_active_transfer`, `mixed_execute_local_to_cloud_directory_move_cancels_during_second_active_transfer`, `mixed_execute_cloud_to_local_directory_move_cancels_during_second_active_transfer`, `attempts local refresh after mixed cloud-to-local failure to reconcile partial writes` | Active directory transfer loops now also have direct execute-phase cancellation coverage in both directions for copy and move; broader partial-summary/hostile-condition breadth still remains open. |
| `CO-MTC-006` | Partial | `mixed_preview_local_to_cloud_matches_onedrive_case_insensitive_and_preserves_kind`, `mixed_preview_cloud_to_local_reports_file_and_dir_conflicts`, `resolves mixed local-to-cloud rename-on-conflict by retrying explicit target candidates`, `resolves mixed cloud-to-local rename-on-conflict for move by retrying target candidates`, `refreshes cloud view after mixed local-to-cloud failure to reconcile partial writes`, `attempts local refresh after mixed cloud-to-local failure to reconcile partial writes`, `mixed_execute_local_to_cloud_directory_copy_cancels_during_second_active_transfer`, `mixed_execute_cloud_to_local_directory_copy_cancels_during_second_active_transfer`, `mixed_execute_local_to_cloud_directory_move_cancels_during_second_active_transfer`, `mixed_execute_cloud_to_local_directory_move_cancels_during_second_active_transfer` | Preview/execute consistency is stronger, including direct execute-phase directory cancellation coverage, but broader partial-summary and hostile-condition assertions still need to be closed for the remaining mixed families. |
...
- Mixed directory loops now also have direct active-cancel coverage in both
  directions for both copy and move:
  `mixed_execute_local_to_cloud_directory_copy_cancels_during_second_active_transfer`,
  `mixed_execute_cloud_to_local_directory_copy_cancels_during_second_active_transfer`,
  `mixed_execute_local_to_cloud_directory_move_cancels_during_second_active_transfer`,
  and `mixed_execute_cloud_to_local_directory_move_cancels_during_second_active_transfer`.
