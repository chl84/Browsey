...
- `hardwareAcceleration` roundtrips through persisted settings, and persisted
  ...
  to `None`.
- `logLevel` remains explicitly covered through dedicated tests because its
  store path applies runtime logging and therefore has a different test seam
  than pure persistence-only settings.
...
## Remaining Gaps Before Step 6 Can Be Fully Closed
