  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'Browsey supports a scoped AQS-like syntax for search. Matching is case-insensitive and evaluated in the backend search stream.',
        bullets: [
          'Wildcards: `*` (many chars) and `?` (single char), for example `filename:*.rs`',
          'Quoted exact phrase: `\"black\"` matches exact basename phrase tokens (for example `black`, not `black-1`)',
          'Exact value with `=` in field values, for example `name:=file.txt`',
          'Boolean logic: `AND`, `OR`, `NOT`, and grouping with parentheses',
          'Supported fields: `name:`, `filename:`, `folder:`, `path:`, `hidden:`, `readonly:`',
          'Examples: `folder:Projects AND filename:*.rs`, `(name:foo OR name:bar) AND NOT hidden:true`',
          'Unsupported in this syntax scope: size/date operators, `is:` aliases, and full Windows AQS compatibility',
        ],
      },
      {
        id: 'column-filters',
