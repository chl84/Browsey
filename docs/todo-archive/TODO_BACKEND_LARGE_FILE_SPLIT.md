...
- [x] If a regression appears, revert only the smallest offending extraction commit. (Protocol defined; not triggered in this wave.)
- [x] Re-run touched domain tests + smoke checklist immediately after rollback. (Protocol defined; not triggered in this wave.)
- [x] Do not proceed to next phase until regression root cause is documented. (Protocol defined; not triggered in this wave.)
...
- [x] Performance drift from moved orchestration (especially listing + thumbnails + cloud refresh). (Tracked: domain timing proxy remained stable; no regression observed.)
- [x] Hidden behavior coupling surfaced by module extraction order. (Tracked: no behavioral drift detected in domain/full suites.)
- [x] Typed-error drift in refactored command seams. (Tracked: hardening guard + clippy clean.)
- [x] Test fragility from parallel fake-rclone and timing-sensitive paths. (Tracked: cloud domain tests stable in this wave.)
- [x] Incomplete smoke coverage for edge providers/archives. (Tracked: automated smoke proxies completed; manual edge-provider/archive pass still recommended before release.)

## Exit / archive

- [x] When all phases are complete, move this file to `docs/todo-archive/`.
- [x] Add completion note (date + final module map + residual risk summary).
...
## Completion note (2026-03-05)

Result:
- Completed the planned commands-first large-file split wave across `thumbnails`, `transfer`, `listing`, `decompress`, `cloud/open`, `cloud/cache`, `settings`, `rename`, `permissions`, plus targeted helper extraction in `compress` and `cloud/rclone_cli`.
- Kept command surfaces stable while reducing single-file responsibility concentration via folder-backed modules and focused helper files.
- Passed full backend quality gates and domain-specific smoke proxies after final splits.

Final module map highlights:
- `settings`: `commands.rs`, `persistence.rs`, `tests.rs`
- `rename`: `ops.rs`, `preview.rs`
- `permissions`: `ownership.rs`, `ownership/unix.rs`, `platform.rs`
- `cloud/open`: `open/inflight.rs`, `open/cache_store.rs`
- `cloud/cache`: `cache/store.rs`, `cache/refresh.rs`
- `decompress/util`: `stream_io.rs`, `path_ops.rs`, `budget.rs`
- `compress`: `pathing.rs`
- `cloud/rclone_cli`: `output.rs`

Residual risk summary:
- No automated regressions detected in typed-error discipline, clippy/format, or full/domain test suites.
- Manual edge smoke for provider-/archive-specific corner-cases remains recommended prior to release tagging.
