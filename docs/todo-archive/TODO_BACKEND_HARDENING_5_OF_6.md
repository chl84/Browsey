...
- [x] `from_external_message(...to_string())` i `src/commands/**`: **0** treff
- [x] `map_err(...to_string())` i `cloud/transfer/network/permissions`: **0** treff
- [x] `impl From<...> for String` i `src/commands/**`: **0** treff (utenfor scope finnes `metadata`/`watcher`)
...
- [x] 0 blocking-funn i Semgrep-scope
- [x] 0 blocking-funn i guard
- [x] API-koder stabile (ingen utilsiktede `code`-endringer)

## 8) Exit + arkivering

- [x] Når alle punkter er grønne: flytt filen til `docs/todo-archive/`
- [x] Legg kort "completion note" i arkivfilen med dato og resultat
