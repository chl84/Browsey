...
- [x] Keep discovery helpers simple, but do not leak raw string errors through command orchestration
...
Tasks:
- [x] Convert top-level extract orchestration to `DecompressResult<_>` end-to-end
- [x] Wrap `spawn_blocking` task results into typed errors at the task boundary, not later
- [x] Introduce typed conversions for archive detection, output-dir preparation, and extraction planning helpers
- [x] Keep format-specific leaf readers string-based only if mapped before returning into `mod.rs`
- [x] Reduce direct `String` error propagation in `util.rs` seams that are used by orchestration
...
Tasks:
- [x] Convert top-level compression orchestration to `CompressResult<_>`
- [x] Wrap `spawn_blocking` task failures into `CompressError` at the task boundary
- [x] Convert planning helpers (`ensure_same_parent`, destination resolution, entry gathering) to typed errors where they cross orchestration boundaries
- [x] Keep low-level zip/IO leaf helpers narrow, but map them before they return to orchestration
...
Tasks:
- [x] Convert `move_ops.rs` orchestration to `FsResult<_>` where it crosses module boundaries
- [x] Convert `delete_ops.rs` blocking/orchestration helpers to `FsResult<_>`
- [x] Keep raw string helpers only for truly local formatting convenience, not module interfaces
- [x] Reuse existing `FsError::from_external_message(...)` less by constructing typed `FsError` earlier
...
- [x] `cargo fmt --all`
- [x] `cargo check -q`
- [x] Module-specific tests stay green after each step
- [x] No change to frontend/backend API contracts unless explicitly required
- [x] No new mixed layers where typed errors are converted back into raw strings

## Commit Strategy

- [x] One focused commit per module family (`thumbnails`, `network`, `decompress`, `compress`, `fs/trash`)
- [x] Separate structural error refactors from behavior changes
- [x] Archive this TODO under `docs/todo-archive/` when the track is complete
