...
use read::cloud_entry_from_item;
#[cfg(test)]
use runtime::{reset_runtime_probe_cache_for_tests, RCLONE_RUNTIME_PROBE_FAILURE_RETRY_BACKOFF};
...
mod tests {
    use super::{
        classify_provider_kind_from_config, classify_provider_rclone_message_code,
        classify_rclone_message_code, is_rclone_not_found_text, map_rclone_error,
        map_rclone_error_for_provider, parse::classify_provider_kind,
        parse::parse_lsjson_items_value, parse::parse_lsjson_stat_item_value,
        parse::parse_rclone_version_stdout, parse::parse_rclone_version_triplet,
    ...
    impl FakeRcloneSandbox {
        ...
        fn provider_with_forced_rc_async_status_error_for_copy(&self) -> RcloneCloudProvider {
            ...
            let rc = RcloneRcClient::new(self.script_path.as_os_str())
                .with_enabled_override_for_tests(true)
                .with_forced_async_status_error_on_copy_for_tests(
                    std::io::ErrorKind::ConnectionReset,
                );
            RcloneCloudProvider { cli, rc }
        }
