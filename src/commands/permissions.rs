...
mod tests {
    ...
    fn read_only_toggle_does_not_grant_world_write() {
        ...
        set_permissions(path.to_string_lossy().to_string(), Some(true), None).unwrap();
        let after_ro = fs::metadata(&path).unwrap().permissions().mode();
        assert_eq!(after_ro & 0o222, 0);

        set_permissions(path.to_string_lossy().to_string(), Some(false), None).unwrap();
    ...
    fn executable_toggle_sets_owner_only() {
        ...
        set_permissions(path.to_string_lossy().to_string(), None, Some(true)).unwrap();
        let after_exec = fs::metadata(&path).unwrap().permissions().mode();
        assert_eq!(after_exec & 0o111, 0o100);

        set_permissions(path.to_string_lossy().to_string(), None, Some(false)).unwrap();
