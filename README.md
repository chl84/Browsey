...
- **Thumbnails**: Lazy, virtualized thumbnail loading in grid view with caching, per-file permission checks, format allowlist, decode timeouts, and size limits; SVGs rasterized with resvg, PDFs rendered client-side with PDF.js, and everything else falls back to icons instantly so UI stays smooth.
